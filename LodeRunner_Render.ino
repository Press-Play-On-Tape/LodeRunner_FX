...
void renderScoreboard() {
  ...
  // Score ..
  Sprites::drawOverwrite(0, 57, HUD_00, 0);
  {
      uint16_t score = player.getScore();
      uint8_t digits[6] = {};
      extractDigits(digits, score);
      
      for(uint8_t i = 0, x = 53; i < 6; ++i, x -= 5) {
        Sprites::drawErase(x, 58, numbers, digits[i]);
      }
  }
  ...
  Sprites::drawOverwrite(61, 57, HUD_01, 0);
  Sprites::drawErase(81, 58, numbers, menLeft / 10);
  Sprites::drawErase(86, 58, numbers, menLeft % 10);
  ...
  if (gameState == GameState::LevelPlay) {
    ...
    Sprites::drawOverwrite(94, 57, HUD_02, 0);
    Sprites::drawErase(117, 58, numbers, goldLeft / 10);
    Sprites::drawErase(122, 58, numbers, goldLeft % 10);

  }
  else {

    uint8_t levelNumber = level.getLevelNumber();
    Sprites::drawOverwrite(94, 57, HUD_03, 0);
    
    const auto divT = div(levelNumber, 100);
    Sprites::drawErase(112, 58, numbers, divT.quot);
    levelNumber = divT.rem;
    Sprites::drawErase(117, 58, numbers, levelNumber / 10);
    Sprites::drawErase(122, 58, numbers, levelNumber % 10);
